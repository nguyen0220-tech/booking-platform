...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleIllegalArgument(IllegalArgumentException e, WebRequest request) {
        ApiResponse<Void> exception = ApiResponse.exception(
                HttpStatus.BAD_REQUEST.value(),
                "INVALID_INPUT",
                e.getMessage(),
    ...
    public ResponseEntity<ApiResponse<Void>> handleOptimisticLockingException(ObjectOptimisticLockingFailureException e, WebRequest request) {
        ApiResponse<Void> exception = ApiResponse.exception(
                HttpStatus.CONFLICT.value(),
                HttpStatus.CONFLICT.getReasonPhrase(),
                "이미 예약된 패키지: " + e.getMessage(),
                request.getDescription(false).replace("uri=", "")
        );
    ...
    public ResponseEntity<ApiResponse<Void>> handleTooManyRequests(TooManyRequestsException e, WebRequest request) {
        ApiResponse<Void> exception = ApiResponse.exception(
                HttpStatus.TOO_MANY_REQUESTS.value(), HttpStatus.TOO_MANY_REQUESTS.getReasonPhrase(),
                "TOO_MANY: " + e.getMessage(),
                request.getDescription(false).replace("uri=", "")
        );

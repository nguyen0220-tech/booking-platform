...
public class FacilityRestaurantUpdate extends AbstractFacilityUpdateHandler<Restaurant, FacilityInfoRestaurantUpdateRequest> {
    ...
    @CacheEvict(value = "facilityRestaurant", allEntries = true)
    @Override
    public ApiResponse<String> updateFacility(Restaurant restaurant, FacilityInfoRestaurantUpdateRequest request) {
        ...
        boolean isOpenTimeNotUpdated = restaurant.getOpenTime().equals(request.getOpenTime());
        boolean isCloseTimeNotUpdated = restaurant.getCloseTime().equals(request.getCloseTime());

        if (isFoodTypeNotUpdated && isOpenTimeNotUpdated && isCloseTimeNotUpdated
                && isNotUpdated(restaurant, request)) {
